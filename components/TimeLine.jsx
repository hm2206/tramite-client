...
const TimeLine = ({ tramite, setLoading }) => {
    const [show_file, setshow_file] = useState(false);
    const [trackingg, settracking] = useState([]);
    let { slug } = tramite
            ...
            </VerticalTimelineElement >
            {
                trackingg.map((track, iter) =>
                    <Fragment key={iter}>
                        ...
                        >
                            <h3 className="vertical-timeline-element-title">{getMetadata(track.status).name || track.status}</h3>
                            <h4 className="vertical-timeline-element-subtitle">{getMetadata(track.status).message}<span className="badge badge-dark">{track && track.dependencia_destino && `${track.dependencia_destino.nombre}`.toUpperCase()}</span></h4>
                            <hr></hr>
                            {track && track.description}

                            <Show condicion={track.files && track.files.length}>
                                <p>
                                    <button className="btn btn-dark btn-sm" onClick={(event) => handleShowFile(track, iter, true)}>Ver Archivos</button>
                                </p>
                            </Show>
