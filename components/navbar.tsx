...
const Navbar: React.FC<NavbarProps> = ({ app }) => {
  ...
  const navBg = isScrolled ? 'bg-white/95 backdrop-blur-lg shadow-lg' : 'bg-transparent';
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link href="/" className="flex items-center space-x-3 group">
            ...
            <div className="flex flex-col">
              <span className={`font-bold text-lg ${textColor} transition-colors duration-300`}>
                {app.name || 'Mesa de Partes'}
              </span>
              <span className={`text-xs ${subtextColor} transition-colors duration-300 hidden sm:block`}>
          ...
          {/* Mobile Menu Button */}
          <button
            className={`md:hidden p-2 rounded-xl ${isScrolled ? 'hover:bg-gray-100' : 'hover:bg-white/10'} transition-colors`}
